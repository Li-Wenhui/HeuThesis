...
- 更傻更简单的选项，例如论文主文件，只需要在文档类的括号中填写本硕博选项，字体选项（设置弹性间距或者刚性间距），文科生选项（目录可以设成四级目录），专业学位类型等，轻松设定目标格式
- 更聪明更简单的自适应格式，例如图题和标题，标题字号在字数超过两行时自动由五号变小五号，实现自适应
...
推荐使用LaTeX安装包自带的中文字体集。

- Linux系统下推荐使用 [fandol](https://www.ctan.org/pkg/fandol)，设置方式：fontset=fandol
- Windows系统推荐使用系统自带字体，设置方式：fontset=windows
...
注意，一般情况下，学位论文的打印版要求双面打印。
博士、硕士论文在双面打印成册时，规范中有明确规定是要右翻页（右翻页是每一章的起始位
置位于书的右侧页面），本科毕业设计论文未作明确要求或限制。
`openright`选项设置为真时，会将所有章（即所有部分，包括前文和后文）起始设置成右翻页。
如果想DIY（或身不由己DIY）在什么地方右翻页，将这个选项设置为false，然后在目标位
置添加`\cleardoublepage`命令即可。

最后向图书管提交的电子版若不是右翻页且要求没有任何空白页，这时只需要设置选项`library=true`
即可，这时候会强制`openright=false`。

### 其他说明

- 本模板是以**哈尔滨工业大学论文模板HiThesis**为基础进行修改。
- 哈尔滨工程大学本科生院和研究生院提供了规范和以及本科生word模板，此模板仅为规范的参考实现，不保证格式审查老师不提意见。任何由于使用本模板而引起的论文格式审查问题均与本模板作者无关
