...
# 模式规则：xeLaTeX → BibTeX → xeLaTeX → xeLaTeX
 $(TARGET): $(TEX_FILES) $(BIB_FILES)
	...
	@echo "正在进行参考文献编译..."
	$(BIBCMD) $(MAIN_TEX)
	@echo "正在进行第二次编译确保，交叉引用正确..."
	$(TEXCMD) $(MAIN_TEX)
	@echo "正在进行第三次编译，确保所有引用稳定..."
	$(TEXCMD) $(MAIN_TEX)
	@echo "正在生成目标文件..."
	$(TEXCMD) $(SPINE)
	$(TEXCMD) $(COVER)
...
help:
	...
	@echo "make cleanall - 清理PDF及所有辅助文件"
