...
\begin{figure}[t]
  ...
  \begin{minipage}{.7\linewidth}
    ...
    \end{minipage}
    \vskip 0.2em
    \wuhao 注意：这里是中文图注添加位置（我校要求，图注在图题之上）。
    \vspace{0.2em}
    \bicaption[golfer47]{}{打高尔夫球的人。注意，此处我校有另外一处要求，子图图题可以位于主图题之下。但由于没有明确说明位于下方具体是什么格式，所以这里不给出举例。}{Fig.$\!$}{The person playing golf. Please note that, although it is appropriate to put subfigures' captions under this caption as stipulated in regulation, but its format is not clearly stated.}
  \end{minipage}
\end{figure}
...
在Windows系统下默认使用系统字体，如果要使用思源字体，需要思源字体的定义文件，此文件请可以自行到网上搜索处下载。
